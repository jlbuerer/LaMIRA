...
sub forwardAlign {
    ...
    $command = "bowtie -v3 -p$threadnum -k1 ".$index." ".$file." --un ".$self->{_outdir}."/unaligned.fastq > ".$self->{_outdir}."/aligned.txt";
    print $command,"\n\n";
    system($command);
...
sub fragmentAlignLeft {
    ...
    for (my $j = $self->{_minfraglen}; $j<= ($self->{_readlen}-$self->{_minfraglen}); $j++){
        ...
        $command = "bowtie -v0 -p$threadnum -a -m1 --trim3 ".$j." ".$index." ".$file." --un ".$remainderfile." --max ".$maxfile." --al ".$alignedfq." > ".$outfile;
        print "\n\n*****\nexecuting: ",$command,"\n";
        system($command);
...
sub fragmentAlignRight {
    ...
    for (my $j = $self->{_minfraglen}; $j<= ($self->{_readlen}-$self->{_minfraglen}); $j++){
        ...
        $command = "bowtie -v0 -p$threadnum -a -m1 --trim5 ".$j." ".$index." ".$file." --un ".$remainderfile." --max ".$maxfile." --al ".$alignedfq." > ".$outfile;
        print "\n\n*****\nexecuting: ",$command,"\n";
        system($command);
